...
import ee.tlu.salat.model.CatFacts;
...
public class RestTemplateController {
    ...
    @GetMapping("get-cat-facts")
    public List<CatFacts> getCatFacts() {
        RestTemplate restTemplate = new RestTemplate();
        String url = "https://cat-fact.herokuapp.com/facts";
        ResponseEntity<CatFacts[]> response = restTemplate.exchange(url, HttpMethod.GET, null, CatFacts[].class);
        return List.of(response.getBody());
    }
    @GetMapping("get-cat-facts/used")
    public List<CatFacts> getCatFactsByUsed() {
        RestTemplate restTemplate = new RestTemplate();
        String url = "https://cat-fact.herokuapp.com/facts";
        ResponseEntity<CatFacts[]> response = restTemplate.exchange(url, HttpMethod.GET, null, CatFacts[].class);
        List<CatFacts> catPosts = new ArrayList<>();
        for (CatFacts c : response.getBody()) {
            if (c.isUsed()) {
                catPosts.add(c);
            }
        }
